                    ...
                    if flag == 1:
                        ...
                        if books[0] > 0:
                            ...
                            if copies[0] < 5:
                                ...
                                iss = datetime.now().date()
                                mycursor.execute(f"update issuedbook set issue_date='{iss}' where b_rfid='{b}' and ub_rfid='{a}'")
                                mydb.commit()

                                print("issue date updated")

                                ret = datetime.now().date()+timedelta(days=15)
                                mycursor.execute(f"update issuedbook set return_date='{ret}' where b_rfid='{b}' and ub_rfid='{a}'")
                                mydb.commit()
